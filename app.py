...
# ----------------------------
def rate_limit(calls_per_minute: int = config.RATE_LIMIT_CALLS_PER_MINUTE): # Corrected attribute name
    def decorator(func):
...
def fmt_number(v, decimals: int = 0, suffix: str = "") -> str: # Added suffix param
    try:
        if pd.isna(v): return "N/A"
...
# ----------------------------
# Custom KPI Card Renderer (Refactored)
# ----------------------------
def render_main_kpi_card(col_container, title, main_value_display_text, delta_display_text=None):
    # main_value_display_text should already be a formatted string (e.g., "SAR 1,234,567", "N/A", "Aug 31, 2025")
    ...
    if delta_display_text is not None and delta_display_text.strip() != "" and delta_display_text.strip().lower() != "n/a":
        delta_color = "#0f172a" 
        if re.match(r'^[+-]', delta_display_text.strip()): # Check if it starts with + or -
...
def main():
            ...
            if df_fm.empty:
                ...
                if not fm_m.empty:
                    ...
                    render_main_kpi_card(kpi_b, "Current", fmt_currency(latest), 
                                         delta_text=f"{mtd_change:,.0f} ({mtd_change_pct:.1f}%)" if pd.notna(mtd_change_pct) else None)
    ...
    with tab_bank:
        ...
        if df_fm.empty:
            ...
            with c1:
                # Ensure data is not empty for charting
                if not df_fm.empty:
    ...
    with tab_fx:
        ...
        if df_fx.empty:
            ...
            if fx_view == "Current Rates":
                ...
                last_update = latest_fx["date"].max() if "date" in latest_fx.columns else pd.NaT # Use NaT for missing date
                last_update_display = last_update.strftime(config.DATE_FMT) if pd.notna(last_update) else "N/A" # Format date string
                render_main_kpi_card(col3, "Last Update", last_update_display, prefix="") # Now a string value
