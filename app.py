...
# (Updated: safe checkbox_multiselect that uses Select ALL/CLEAR buttons to avoid session_state write-to-widget conflicts)
...
# ----------------------------
# Checkbox-multiselect Helper (SAFE)
# ----------------------------
def _sanitize_key(s: str) -> str:
    return re.sub(r'[^0-9a-zA-Z]+', '_', str(s).strip()).lower()

def checkbox_multiselect(options: list, key_prefix: str, cols: int = 4, default_all: bool = True, show_select_clear: bool = True):
    """
    Render a grid of checkboxes with stable keys and provide 'ALL' and 'CLEAR' buttons.
    - Does NOT write to widget-owned keys outside of callbacks (avoids StreamlitAPIException).
    ...
    per_keys = [f"{key_prefix}_{safe}" for _, safe in sanitized]

    # Initialize per-option session keys (ok to set before widget is created)
    for pk in per_keys:
        if pk not in st.session_state:
            st.session_state[pk] = bool(default_all)

    # Buttons for ALL / CLEAR that run in their own click lifecycle (safe to set widget-state keys)
    if show_select_clear:
        bcol1, bcol2 = st.columns([1, 6])
        with bcol1:
            if st.button("ALL", key=f"{key_prefix}_btn_all"):
                for pk in per_keys:
                    st.session_state[pk] = True
        with bcol2:
            if st.button("CLEAR", key=f"{key_prefix}_btn_clear"):
                for pk in per_keys:
                    st.session_state[pk] = False

    # Render options in grid using stable keys
    ncols = min(cols, max(1, len(options)))
    col_layout = st.columns(ncols)
    selected = []
    for i, (opt, safe) in enumerate(sanitized):
        pk = f"{key_prefix}_{safe}"
        checked = col_layout[i % ncols].checkbox(opt, value=st.session_state.get(pk, False), key=pk)
        if checked:
            selected.append(opt)
    ...
    with tab_export_lc:
        ...
        if df_export_lc.empty:
            ...
            with col1:
                ...
                # Use safe checkbox grid with ALL/CLEAR buttons
