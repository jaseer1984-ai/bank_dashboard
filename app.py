...
# - ✅ Colored tabs (each tab has its own background color)
...
/* Tab colors (professional palette) */
.stTabs [data-baseweb="tab"]:nth-child(1){ background:#ffe8ec; color:#9f1239; }   /* Overview = rose */
.stTabs [data-baseweb="tab"]:nth-child(2){ background:#e8f0ff; color:#1e3a8a; }   /* Bank = indigo */
.stTabs [data-baseweb="tab"]:nth-child(3){ background:#e9fce7; color:#065f46; }   /* Settlements = emerald */
.stTabs [data-baseweb="tab"]:nth-child(4){ background:#fff3e0; color:#92400e; }   /* Supplier = amber */
.stTabs [data-baseweb="tab"]:nth-child(5){ background:#f2e8ff; color:#6d28d9; }   /* FX = purple */
.stTabs [data-baseweb="tab"]:nth-child(6){ background:#e6fffb; color:#0f766e; }   /* Facility = teal */
...
def main():
    ...
    # ===== Tabs =====
    tab_overview, tab_bank, tab_settle, tab_pay, tab_fx, tab_facility = st.tabs(
        ["Overview", "Bank", "Settlements", "Supplier Payments", "Exchange Rate", "Facility Report"]
    )

    # ---------------- Overview ----------------
    with tab_overview:
        st.markdown('<span class="section-chip">📅 Monthly Overview — Detailed Insights</span>', unsafe_allow_html=True)

        # Determine current month span
        month_start = today0.replace(day=1)
        month_end = (month_start + pd.offsets.MonthEnd(1)).to_pydatetime()

        # High-level KPIs for the month
        colA, colB, colC, colD = st.columns(4)
        with colA: st.metric("Banks (Active)", banks_cnt)
        with colB: st.metric("Total Balance (Now)", fmt_number_only(total_balance))
        with colC: st.metric("Approved Payments (All)", fmt_number_only(approved_sum))
        with colD: st.metric("LC Due (Next 4d)", fmt_number_only(lc_next4_sum))

        st.markdown("")

        # Liquidity (month)
        st.markdown("**📈 Liquidity — This Month**")
        if not df_fm.empty:
            fm_month = df_fm[df_fm["date"].between(month_start, month_end)]
            if not fm_month.empty:
                try:
                    ...
                    fig = go.Figure()
                    fig.add_trace(go.Scatter(x=fm_month["date"], y=fm_month["total_liquidity"], mode="lines+markers", line=dict(width=3)))
                    fig.update_layout(template="brand", height=300, margin=dict(l=10,r=10,t=20,b=20),
                                      xaxis_title=None, yaxis_title="Liquidity (SAR)")
                    fig.update_xaxes(rangeslider_visible=False, rangeselector=None, tickformat="%b %d")
                    st.plotly_chart(fig, use_container_width=True)
                except Exception:
                    st.line_chart(fm_month.set_index("date")["total_liquidity"])
            else:
                st.info("No liquidity points for this month.")
        else:
            st.info("No liquidity data available.")

        st.markdown("---")

        # Bank concentration & negatives
        st.markdown("**🏦 Bank Concentration & Alerts (Current)**")
        if not df_by_bank.empty:
            c1, c2 = st.columns([2,1])
            with c1:
                tmp = df_by_bank.sort_values("balance", ascending=False)
                share = tmp.assign(share=lambda d: d["balance"]/d["balance"].sum()*100)[["bank","balance","share"]]
                tbl = share.rename(columns={"bank":"Bank","balance":"Balance","share":"Share %"})
                st.dataframe(style_right(tbl, num_cols=["Balance","Share %"], decimals=1), use_container_width=True, height=300)
            with c2:
                neg_rows = df_by_bank[df_by_bank["balance"] < 0]
                if neg_rows.empty:
                    st.success("No banks with negative available balance.")
                else:
                    st.error(f"{len(neg_rows)} bank(s) negative:")
                    for _, r in neg_rows.iterrows():
                        st.write(f"• **{r['bank']}** — {fmt_number_only(r['balance'])}")
        else:
            st.info("No bank balance data.")

        st.markdown("---")

        # Payments & LC due within month
        st.markdown("**💸 This Month: Payments & LC Due**")
        col1, col2 = st.columns(2)
        with col1:
            if not df_pay_approved.empty:
                pay_m = df_pay_approved.copy()
                st.metric("Approved (count)", len(pay_m))
                st.metric("Approved (total)", fmt_number_only(pay_m["amount"].sum()))
                grp = (pay_m.groupby("bank", as_index=False)["amount"]
                       .sum().sort_values("amount", ascending=False)
                       .rename(columns={"bank":"Bank","amount":"Amount"}))
                st.dataframe(style_right(grp, num_cols=["Amount"]), use_container_width=True, height=250)
            else:
                st.info("No approved payments.")
        with col2:
            if not df_lc.empty:
                lc_m = df_lc[df_lc["settlement_date"].dt.to_period("M") == today0.to_period("M")]
                st.metric("LCs due this month", len(lc_m))
                st.metric("Amount (this month)", fmt_number_only(lc_m["amount"].sum()))
                if not lc_m.empty:
                    show = lc_m[["bank","settlement_date","amount"]].rename(columns={"bank":"Bank","settlement_date":"Settlement Date","amount":"Amount"})
                    show["Settlement Date"] = show["Settlement Date"].dt.strftime(config.DATE_FMT)
                    st.dataframe(style_right(show, num_cols=["Amount"]), use_container_width=True, height=250)
            else:
    ...
    with tab_pay:
        ...
        tabA, tabB = st.tabs(["Approved", "Released"])
        with tabA:
    ...
    # ---------------- Facility Report ----------------
    with tab_facility:
        st.info("📄 Facility Report: add your facility exposure/utilization content here.")
