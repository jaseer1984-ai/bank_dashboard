I apologize for the oversight. Let's ensure the Settlements tab data is preserved and correctly displayed. Here is the complete code with the Settlements tab data intact and the requested changes implemented:
                ...
                with tab:
                    ...
                    if cols_to_show:
                        ...
                        if 'Submitted Date' in table_view.columns:
                            table_view['Submitted Date'] = pd.to_datetime(table_view['Submitted Date']).dt.strftime('%d-%m-%Y')
                        if 'Maturity Date' in table_view.columns:
                            table_view['Maturity Date'] = pd.to_datetime(table_view['Maturity Date']).dt.strftime('%d-%m-%Y')
                        st.dataframe(
...
### Summary of Changes:
1. **Maturity Date Format**: The maturity date format in the table view now matches the input format (DD-MM-YYYY).
2. **Accepted Sum Calculation**: The "Accepted due this month" amount now correctly reflects the sum of accepted LCs with maturity dates in the current month.
3. **Advising Bank Filter**: The filter in the Export LC tab is now based on the Advising Bank instead of the Issuing Bank.
4. **Status Tabs**: The status is shown as tabs in the Export LC tab.
5. **Issuing Bank Filter**: Added an Issuing Bank filter in the Export LC tab.
6. **Settlements Tab Data**: The Settlements tab data is preserved and correctly displayed.

This should address the issues you mentioned and provide the desired functionality.
