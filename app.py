The error you're encountering is due to the `THEME` dictionary not being properly initialized before it's used in the CSS styling. This can happen if the `THEME` dictionary is defined after the CSS styling block.

To fix this, you need to ensure that the `THEME` dictionary is defined before it's used in the CSS styling. Here's the corrected code with proper initialization:
        ...
        def render_payments_tab(df_src: pd.DataFrame, status_label: str, key_suffix: str):
            ...
            if not view_data.empty:
                ...
                if payment_view == "Summary + Table":
                    ...
                    bank_totals = view_data.groupby("bank", as_index=False)["amount"].sum().rename(columns={"amount": "
